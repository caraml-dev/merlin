...
install_merlin() {
  ...
  helm upgrade --install --debug merlin ${CHART_PATH} --namespace=mlp --create-namespace -f ${CHART_PATH}/values-e2e.yaml \
    ...
    --set merlin.imageBuilder.buildContextURI=git://github.com/gojek/merlin.git#${GIT_REF} \
    --set merlin.imageBuilder.predictionJobBuildContextURI=git://github.com/gojek/merlin.git#${GIT_REF} \
    --set-json 'merlin.imageBuilder.predictionJobBaseImage={"3.7.*":"'${DOCKER_REGISTRY}'/merlin/merlin-pyspark-base-py37:'${VERSION}'","3.8.*":"'${DOCKER_REGISTRY}'/merlin/merlin-pyspark-base-py38:'${VERSION}'","3.9.*":"'${DOCKER_REGISTRY}'/merlin/merlin-pyspark-base-py39:'${VERSION}'","3.10.*":"'${DOCKER_REGISTRY}'/merlin/merlin-pyspark-base-py310:'${VERSION}'"}' \
