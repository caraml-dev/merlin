...
# Add `merlin-batch-predictor` to conda's pip dependencies, if not exist
yq --inplace 'with(.dependencies[].pip; select(all_c(. != "*merlin-batch-predictor*")) | . += ["merlin-batch-predictor==0.40.2.dev29"] )' "${CONDA_ENV_PATH}"
