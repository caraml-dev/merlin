  ...
  const columns = [
    {
      ...
      render: (name, item) => (
        <Link
          to={`/merlin/projects/${item.project_id}/models/${item.model_id}/versions/${item.version_id}/jobs/${item.id}/details`}
    ...
    },
    {
      ...
      name: "Model Version",
      width: "10%",
      render: (version_id, item) => (
        <Link
          to={`/merlin/projects/${item.project_id}/models/${item.model_id}/versions/${version_id}`}
          onClick={(e) => e.stopPropagation()}
        >
      ...
      render: (_, item) => (
        <EuiFlexGroup alignItems="flexStart" direction="column" gutterSize="xs">
          ...
          <EuiFlexItem grow={false} key={`stop-job-${item.id}`}>
            ...
            >
              <EuiText size="xs">
                {isActiveJob(item.status) ? "Terminate" : "Delete"}
              </EuiText>
            </EuiButtonEmpty>
          </EuiFlexItem>
        </EuiFlexGroup>
